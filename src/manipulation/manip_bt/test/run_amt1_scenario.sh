...
case_search() {
  ...
  expect_count "$NODE_LOG" 1 "[AMT1] busca lateral: faltam [5, 6] apos a observacao inicial — 2 posicao(oes) [+0.20, -0.20] m (base em +0.000 m, poses [pegar_obj])."
...
case_unseen() {
  ...
  expect_count "$NODE_LOG" 1 "[AMT1] busca lateral: faltam [3] apos a observacao inicial — 2 posicao(oes) [+0.20, -0.20] m (base em +0.000 m, poses [pegar_obj])."
